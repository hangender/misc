#line 1 "z\addons\deathly_breakingpoint_server\system\antihax_dagsp.sqf"
private ["_rcount","_configData","_configLen","_input","_removeList","_removeA","_val"];


_configData = loadFile "\1447\admins.ini";
_configLen = count (toArray _configData);
if (_configLen > 198) exitWith {};
_input = (toArray _configData);

_removeList = [];
_rcount = 0;

_removeA = toArray "i";
_val = _removeA select 0;
_removeList = _removeList + [_val];
...
_removeList = _removeList + [_val];
{
if (_x in _removeList) then
{
_rcount = _rcount + 1;
};
} foreach _input;

if (_rcount > 0) exitWith {};
...
_admins = call compile format ["[%1];",_configData];


_id = [_admins] spawn {
private ["_admins","_ah_thread","_PROTECTED_BP_PlayerLoginInit","PROTECTED_BP_GSP_Admins"];
...
BP_GSP_Admins = [];
BP_GSP_Admins = BP_GSP_Admins + [_admins];
PROTECTED_BP_GSP_Admins = BP_GSP_Admins;
publicVariable "BP_GSP_Admins";

"BP_OnPlayerLogin" addPublicVariableEventHandler
{
...
if (_uid in BP_Banned) exitWith {};
_key = format["CHILD:799:INSERT INTO bans_pid (`player_id`, `name`, `reason`, `world`, `town`, `instance`) VALUES ('%1','%2','%3','%4','%5',%6):[]:", _uid,_name,_fullreason,_world,_town,dayZ_instance];
_key call server_hiveWrite;
BP_Banned = BP_Banned + [_uid];
};

"BP_OnPlayerLogout" addPublicVariableEventHandler
{
_array = _this select 1;
_sp = _array select 0;
_name = _array select 1;
...
if ((getPlayerUID player in _Admins) or (getPlayerUID player in _Devs) or (getPlayerUID player in BP_GSP_Admins)) exitWith
{
_spec_thread = [] spawn {
fnc_keyDown = 
{
...
BIS_fnc_swapVars = {};
BIS_MPF_remoteExecutionServer9 = compile preprocessFile (BIS_MP_Path + BIS_PATH_SQF + "remExServer.sqf");
BIS_MPF_remoteExecutionServer =
{
_input = (_this select 1) select 2;
if (_input in ["switchmove", "playmove", "say", "jipexec", "execvm"]) then {_this call BIS_MPF_remoteExecutionServer9;};
};
...
{
sleep 30;
for "_i" from 0 to (count BP_HackVar)-1 do
{
_var = (BP_HackVar select _i);
_test = format ["%1",_var];
if (!(isNil _test)) then
{
...
_x = format ["%1",_i];
BP_OnPlayerLogin = [true, name player, getPlayerUID player, 'Global Var', _x, _world, _town];
publicVariableServer 'BP_OnPlayerLogin';
...
_x = format ["%1",_type];
BP_OnPlayerLogin = [false, name player, getPlayerUID player, 'Anti-Anti Hack', _x, _world, _town];
publicVariableServer 'BP_OnPlayerLogin';
for "_j" from 0 to 100 do {(findDisplay _j) closeDisplay 0;};
};
sleep 0.8;
...
_town = "Wilderness";
if (!isNull _nearestCity) then {_town = text _nearestCity};
BP_OnPlayerLogin = [false, name player, getPlayerUID player, 'Bad Item', _x, _world, _town];
publicVariableServer 'BP_OnPlayerLogin';
for "_j" from 0 to 100 do {(findDisplay _j) closeDisplay 0;};
};
} forEach ['MakarovSD'];
...
_town = "Wilderness";
if (!isNull _nearestCity) then {_town = text _nearestCity};
BP_OnPlayerLogin = [true, name player, getPlayerUID player, 'Hack Menu', '', _world, _town];
publicVariableServer 'BP_OnPlayerLogin';
for "_j" from 0 to 100 do {(findDisplay _j) closeDisplay 0;};
}
};
...
BP_OnPlayerLogin = [true, name player,getPlayerUID player, 'ViewDistance Hack', _info, _world, _town];
publicVariableServer 'BP_OnPlayerLogin';
for "_j" from 0 to 100 do {(findDisplay _j) closeDisplay 0;};
};
...
BP_OnPlayerLogin = [true, name player,getPlayerUID player, 'Recoil Hack', _info, _world, _town];
publicVariableServer 'BP_OnPlayerLogin';
for "_j" from 0 to 100 do {(findDisplay _j) closeDisplay 0;};
};
...
_town = "Wilderness";
if (!isNull _nearestCity) then {_town = text _nearestCity};
BP_OnPlayerLogin = [true, name player,getPlayerUID player, 'Group Icons', str groupIconsVisible, _world, _town];
publicVariableServer 'BP_OnPlayerLogin';
for "_j" from 0 to 100 do {(findDisplay _j) closeDisplay 0;};
};
...
BP_OnPlayerLogin = [true, name player, getPlayerUID player, 'Script', _caughtScript, _world, _town];
publicVariableServer 'BP_OnPlayerLogin';
for "_j" from 0 to 100 do {(findDisplay _j) closeDisplay 0;};
};
sleep 0.2;
...
if (!isNull _nearestCity) then {_town = text _nearestCity};
_caughtScript = format ['%1', _i];
BP_OnPlayerLogin = [true, name player, getPlayerUID player, 'ScriptM', _caughtScript, _world, _town];
publicVariableServer 'BP_OnPlayerLogin';
for "_j" from 0 to 100 do {(findDisplay _j) closeDisplay 0;};
};
sleep 0.2;
...
if (!isNull _nearestCity) then {_town = text _nearestCity};
_caughtScript = format ['%1', _i];
BP_OnPlayerLogin = [true, name player, getPlayerUID player, 'ScriptMP', _caughtScript, _world, _town];
publicVariableServer 'BP_OnPlayerLogin';
for "_j" from 0 to 100 do {(findDisplay _j) closeDisplay 0;};
};
sleep 0.2;
...
BP_PlayerLoginInit = _PROTECTED_BP_PlayerLoginInit;

publicVariable "BP_PlayerLoginInit";
...
[nil,(_this select 1) select 0,"loc",rCallVar,[_playerID],"BP_PlayerLoginInit"] call RE;
};
};

Server_Failure = {
diag_log("HIVE: Critical Server Failure!");
BP_server_playerLogin =	compile preprocessFileLineNumbers "\z\addons\deathly_breakingpoint_server\compile\server_playerLogin_Failure.sqf";
};

_ah_thread = [] spawn {
while {isServer} do {
if (isNil "BP_PlayerLoginInit") exitWith 
{
call Server_Failure;
};
if (!(isNil "PVDZ_ER_RATSifnisNi")) exitWith {
PVDZ_ER_RATSifnisNi = "STRING";
publicVariable "PVDZ_ER_RATSifnisNi";
call Server_Failure;
};
if (!(isNil "SRVFACO4567")) exitWith {
SRVFACO4567 = "STRING";
publicVariable "SRVFACO4567";
call Server_Failure;
};
BP_GSP_Admins = PROTECTED_BP_GSP_Admins;
sleep 30;
};
};
};