private ["_target", "_caller","_isNearFeed"];
...
{
	_isNearFeed = count ((position _caller) nearObjects ["Land_A_FuelStation_Shed", 15]) > 0;

	if (!_isNearFeed) then
	{
		titleText ["You must be near a Fuelstation.", "PLAIN DOWN", 3];
		titleFadeOut 3;
	}
	else
	{
