...
//Stream in objects
	/* STREAM OBJECTS */
		...
		_countr = 0;		
		{
			...
			diag_log ("_type = " + str(_type) + "_dobav = " + str(_dobav));
			...
			if (_damage < 1) then {
				...
				if (count _worldspace > 2) then
				{
					...
					diag_log format ["vector = %1, typeOf=%2", _worldspace select 2, typeOf _object];
				};
				...
				{
					_levelhouse = 0;
					diag_log format ["_idKey=%1",_idKey];
					{
						_selection = _x select 0;
						_dam = _x select 1;
						if(_selection !="passwordtut") then {
							if (_dam >= 9999999) then {
								...
								diag_log format ["Name = %1",_selection];
							} else {
								_object animate [_selection,_dam];
								//bandit
						...
						diag_log format ["_selection=%1,_dam=%2,lvl=%3",_selection,_dam,_levelhouse];
					} forEach _hitpoints;
				...
				if (!_dobav) then {
					_newinvhouse set [count _newinvhouse,_object];
				};
			};
		} forEach _myArray;
...
// preload func
	...
	_weights = [];
// call this func only once, because _weights is same.
...
{ 
	[_x,"gear"] call server_updatObiect;
	diag_log format ["%1, %2", _x, typeOf _x];

} foreach _newinvhouse;
...
start_guardsg01g = {
   ...
   _modelarrayg01g = ["SectorB_def","SectorB_def","SectorB_def"];
    ...
    _rndCountg01g = 75; //ceil((random 3) + 1); // no soldiers
       ...
       _bodynameig01g = format ["TavianaGuardg01g%1",_x];
        ...
        if(_x == 1) then {
			...
			if((_x >= 7) and (_x <= 11)) then {
				_spawnAISg01g addweapon "DMR";
				_spawnAISg01g addMagazine "20Rnd_762x51_DMR";
				_spawnAISg01g addMagazine "20Rnd_762x51_DMR";
				_spawnAISg01g addMagazine "20Rnd_762x51_DMR";
				_spawnAISg01g addMagazine "20Rnd_762x51_DMR";
				_spawnAISg01g addMagazine "20Rnd_762x51_DMR";
				_spawnAISg01g addMagazine "20Rnd_762x51_DMR";
				_spawnAISg01g selectWeapon "DMR";
			};
...
start_civilian1c = {
   ...
   _modelarray1c = ["Villager1","Citizen1","Citizen2","Citizen3","Citizen4","Profiteer1","Profiteer2","Profiteer3","Profiteer4","Secretary1","Secretary2","Secretary3","Secretary4"];
    ...
    _rndCount1c = 11; //ceil((random 3) + 1); // no soldiers
    ...
    for "_x" from 1 to _rndCount1c do {
       ...
       _bodynamei1c = format ["TavianaCivil1c%1",_x];
        ...
        if(_x == 1) then {  
			_soldermodel1c = _modelarray1c select 0; //(round(random ((count _modelarray1c) - 1)));
            ...
            //_spawnTaviGroupLeader1c addmpeventhandler ["Killed",{_this call delete_guardia;}];
            ...
            //waituntil {!isnil "bis_fnc_init"};
            //[_AISgroup1c, [(_position select 0),(_position select 1),0], 450] call bis_fnc_taskPatrol;
			//[_AISgroup1c, [(_position select 0),(_position select 1),0]] call BIS_fnc_taskDefend;
			//_id101 = [_AISgroup1c, [(_position select 0),(_position select 1),0]] execVM "\z\addons\dayz_server\system\BIN_taskDefend.sqf";
			...
			if(_x == 2) then {
				...
				_spawnAIS1c setDir 0; diag_log ("2");
			};
			if(_x == 3) then {
...
waitUntil{!isNil "BIS_fnc_selectRandom"};
   ...
   _modelarrayg13g = ["SectorZ_def","SectorZ_def","SectorZ_def"];
    ...
    _rndCountg13g = 11; //ceil((random 3) + 1); // no soldiers
    _rnd = random 1; // chance of highend loot on leader
    for "_x" from 1 to _rndCountg13g do {
       ...
       _bodynameig13g = format ["TavianaGuardg13g%1",_x];
        ...
        if(_x == 1) then {
            ...
            //waituntil {!isnil "bis_fnc_init"};
            //[_AISgroupg13g, [14844.3,9927.72,2.8672], 20] call bis_fnc_taskPatrol;
			//[_AISgroupg13g, [14849.2,9924.56,2.74393]] call BIS_fnc_taskDefend;
			//null = [_AISgroupg13g, [(_position select 0),(_position select 1),0]] execVM "\z\addons\dayz_server\system\BIN_taskDefend.sqf";
			...
			if(_x == 2) then {
				_spawnAISg13g addweapon "Pecheneg_DZ";
				_spawnAISg13g addMagazine "100Rnd_762x54_PK";
				_spawnAISg13g addMagazine "100Rnd_762x54_PK";
				_spawnAISg13g addMagazine "100Rnd_762x54_PK";
				_spawnAISg13g addMagazine "100Rnd_762x54_PK";
				_spawnAISg13g addMagazine "100Rnd_762x54_PK";
				_spawnAISg13g selectWeapon "Pecheneg_DZ";
			};
			if(_x == 3) then {
				_spawnAISg13g addweapon "DMR";
				_spawnAISg13g addMagazine "20Rnd_762x51_DMR";
				_spawnAISg13g addMagazine "20Rnd_762x51_DMR";
				_spawnAISg13g addMagazine "20Rnd_762x51_DMR";
				_spawnAISg13g addMagazine "20Rnd_762x51_DMR";
				_spawnAISg13g addMagazine "20Rnd_762x51_DMR";
				_spawnAISg13g selectWeapon "DMR";
			};
			if(_x == 4) then {
...
/*
start_guardsg17g = {
	private["_spawnTaviGroupLeaderg17g","_AISgroupg17g","_x10","_rndCountg17g","_rnd10","_positiong17g","_modelarrayg17g","_startposg17g","_unit10","_units10","_id101"];
	waitUntil{!isNil "BIS_fnc_selectRandom"};
	waituntil {!isnil "bis_fnc_init"};
    ...
    _AISgroupg17g = createGroup east;   
   _modelarrayg17g = ["SectorB_def","SectorB_def","SectorB_def"];
					
    _startposg17g = [14849.2,9924.56,2.74393];
    //_position = [8080,6618];
	_positiong17g = [14849.2,9924.56,2.74393];
    _rndCountg17g = 10; //ceil((random 3) + 1); // no soldiers
    _rnd = random 1; // chance of highend loot on leader
    for "_x" from 1 to _rndCountg17g do {
        private["_spawnAISg17g","_bodynameig17g","_soldermodelg17g","_spawnTaviGroupLeaderg17g"];
       _bodynameig17g = format ["TavianaGuardg17g%1",_x];
	   
        if(_x == 1) then {
			_soldermodelg17g = _modelarrayg17g select (round(random ((count _modelarrayg17g) - 1)));
            _soldermodelg17g createunit [[14849.2,9924.56,2.74393], _AISgroupg17g, "_spawnTaviGroupLeaderg17g = this; TavianaSquadg17g = group this;"];
			_spawnTaviGroupLeaderg17g setPos [14849.2,9924.56,2.74393];
            _spawnTaviGroupLeaderg17g enableAI "TARGET";
            _spawnTaviGroupLeaderg17g enableAI "AUTOTARGET";
            _spawnTaviGroupLeaderg17g enableAI "MOVE";
            _spawnTaviGroupLeaderg17g enableAI "ANIM";
            _spawnTaviGroupLeaderg17g enableAI "FSM";
            _spawnTaviGroupLeaderg17g allowDammage true;
            _spawnTaviGroupLeaderg17g setCombatMode "RED";
            _spawnTaviGroupLeaderg17g setBehaviour "COMBAT";
            _spawnTaviGroupLeaderg17g setSkill ["aimingAccuracy",1];
            _spawnTaviGroupLeaderg17g setSkill ["aimingShake",1];
            _spawnTaviGroupLeaderg17g setSkill ["aimingSpeed",1];
            _spawnTaviGroupLeaderg17g setSkill ["commanding",1];
			_spawnTaviGroupLeaderg17g setSkill ["aimingAccuracy",1];
			_spawnTaviGroupLeaderg17g setSkill ["endurance",1];
			_spawnTaviGroupLeaderg17g setSkill ["spotDistance",1];
			_spawnTaviGroupLeaderg17g setSkill ["spotTime",1];
			_spawnTaviGroupLeaderg17g setSkill ["courage",1];
			_spawnTaviGroupLeaderg17g setSkill ["reloadSpeed",1];
			_spawnTaviGroupLeaderg17g setSkill ["general",1];
			
            _spawnTaviGroupLeaderg17g addMagazine "ItemBandage";
            _spawnTaviGroupLeaderg17g addMagazine "ItemBandage";           
            _spawnTaviGroupLeaderg17g addweapon "NVGoggles";
            _spawnTaviGroupLeaderg17g addweapon "ItemCompass";
            _spawnTaviGroupLeaderg17g addweapon "ItemMap";
            _spawnTaviGroupLeaderg17g addweapon "ItemGPS";
            _spawnTaviGroupLeaderg17g addweapon "ItemKnife";
            _spawnTaviGroupLeaderg17g addweapon "ItemMatchbox";
            
            _spawnTaviGroupLeaderg17g addweapon "Sa58V_EP1";
            _spawnTaviGroupLeaderg17g addMagazine "30Rnd_762x39_SA58";
            _spawnTaviGroupLeaderg17g addMagazine "30Rnd_762x39_SA58";
            _spawnTaviGroupLeaderg17g addMagazine "30Rnd_762x39_SA58";
            _spawnTaviGroupLeaderg17g addMagazine "30Rnd_762x39_SA58";
			_spawnTaviGroupLeaderg17g addMagazine "30Rnd_762x39_SA58";
			_spawnTaviGroupLeaderg17g addMagazine "30Rnd_762x39_SA58";
            _spawnTaviGroupLeaderg17g selectWeapon "Sa58V_EP1";
            _spawnTaviGroupLeaderg17g addEventHandler ["Fired", {_this call player_fired;}];
			_spawnTaviGroupLeaderg17g addmpeventhandler ["Killed",{_this call delete_guardia;}];
			_spawnTaviGroupLeaderg17g setVariable ["bodyname",_bodynameig17g,false];
			_spawnTaviGroupLeaderg17g setVariable ["delmeplease",_bodynameig17g,false];
			_spawnTaviGroupLeaderg17g disableConversation true;
			_spawnTaviGroupLeaderg17g setDir 274;
			_spawnTaviGroupLeaderg17g setdamage 0.01;
		   
           
            //[_AISgroupg17g, [(_position select 0),(_position select 1),0], 450] call bis_fnc_taskPatrol;
			//[TavianaSquadg17g, [(_position select 0),(_position select 1),(_position select 2)]] call BIS_fnc_taskDefend;
			//null = [_AISgroupg17g, [(_position select 0),(_position select 1),0]] execVM "\z\addons\dayz_server\system\BIN_taskDefend.sqf";
			
        } else {
			_soldermodelg17g = _modelarrayg17g select (round(random ((count _modelarrayg17g) - 1)));
            if(_x == 2) then {
				_soldermodelg17g createunit [[14849.3,9929.21,2.8459], _AISgroupg17g, "_spawnAISg17g = this;"];
				_spawnAISg17g setPos [14849.3,9929.21,2.8459];
				_spawnAISg17g setDir 259;
            };
			if(_x == 3) then {
				_soldermodelg17g createunit [[14846.7,9934.32,2.88913], _AISgroupg17g, "_spawnAISg17g = this;"];
				_spawnAISg17g setPos [14846.7,9934.32,2.88913];
				_spawnAISg17g setDir 268;
            };
			if(_x == 4) then {
				_soldermodelg17g createunit [[14838.4,9916.73,2.96353], _AISgroupg17g, "_spawnAISg17g = this;"];
				_spawnAISg17g setPos [14838.4,9916.73,2.96353];
				_spawnAISg17g setDir 294;
            };
			if(_x == 5) then {
				_soldermodelg17g createunit [[14810.6,9933.96,2.90491], _AISgroupg17g, "_spawnAISg17g = this;"];
				_spawnAISg17g setPos [14810.6,9933.96,2.90491];
				_spawnAISg17g setDir 105;
            };
			if(_x == 6) then {
				_soldermodelg17g createunit [[14813.8,9927.45,6.611], _AISgroupg17g, "_spawnAISg17g = this;"];
				_spawnAISg17g setPos [14813.8,9927.45,6.611];
				_spawnAISg17g setDir 260;
            };
			if(_x == 7) then {
				_soldermodelg17g createunit [[14813.8,9912.71,6.52775], _AISgroupg17g, "_spawnAISg17g = this;"];
				_spawnAISg17g setPos [14813.8,9912.71,6.52775];
				_spawnAISg17g setDir 0;
            };
			if(_x == 8) then {
				_soldermodelg17g createunit [[14841.7,9922.65,6.44907], _AISgroupg17g, "_spawnAISg17g = this;"];
				_spawnAISg17g setPos [14841.7,9922.65,6.44907];
				_spawnAISg17g setDir 295;
            };
			if(_x == 9) then {
				_soldermodelg17g createunit [[14840.7,9922.65,6.44907], _AISgroupg17g, "_spawnAISg17g = this;"];
				_spawnAISg17g setPos [14841.7,9922.65,6.44907];
				_spawnAISg17g setDir 295;
            };
			if(_x == 10) then {
				_soldermodelg17g createunit [[14842.7,9922.65,6.44907], _AISgroupg17g, "_spawnAISg17g = this;"];
				_spawnAISg17g setPos [14841.7,9922.65,6.44907];
				_spawnAISg17g setDir 295;
            };
			
			_spawnAISg17g enableAI "TARGET";
            _spawnAISg17g enableAI "AUTOTARGET";
            _spawnAISg17g enableAI "MOVE";
            _spawnAISg17g enableAI "ANIM";
            _spawnAISg17g enableAI "FSM";
            _spawnAISg17g allowDammage true;
            _spawnAISg17g setCombatMode "RED";
            _spawnAISg17g setBehaviour "COMBAT";
            _spawnAISg17g setSkill ["aimingAccuracy",1];
            _spawnAISg17g setSkill ["aimingShake",1];
            _spawnAISg17g setSkill ["aimingSpeed",1];
            _spawnAISg17g setSkill ["commanding",1];
			_spawnAISg17g setSkill ["aimingAccuracy",1];
			_spawnAISg17g setSkill ["endurance",1];
			_spawnAISg17g setSkill ["spotDistance",1];
			_spawnAISg17g setSkill ["spotTime",1];
			_spawnAISg17g setSkill ["courage",1];
			_spawnAISg17g setSkill ["reloadSpeed",1];
			_spawnAISg17g setSkill ["general",1];
			...
			if(_x == 2) then {
				_spawnAISg17g addweapon "AK_47_M";
				_spawnAISg17g addMagazine "30Rnd_762x39_AK47";
				_spawnAISg17g addMagazine "30Rnd_762x39_AK47";
				_spawnAISg17g addMagazine "30Rnd_762x39_AK47";
				_spawnAISg17g addMagazine "30Rnd_762x39_AK47";
				_spawnAISg17g selectWeapon "AK_47_M";
			};
			if(_x == 3) then {
				_spawnAISg17g addweapon "Pecheneg_DZ";
				_spawnAISg17g addMagazine "100Rnd_762x54_PK";
				_spawnAISg17g addMagazine "100Rnd_762x54_PK";
				_spawnAISg17g addMagazine "100Rnd_762x54_PK";
				_spawnAISg17g addMagazine "100Rnd_762x54_PK";
				_spawnAISg17g selectWeapon "Pecheneg_DZ";
			};
			if(_x == 4) then {
				_spawnAISg17g addweapon "AK_47_M";
				_spawnAISg17g addMagazine "30Rnd_762x39_AK47";
			...
			_spawnAISg17g setVariable ["bodyname",_bodynameig17g,false];		
			_spawnAISg17g setVariable ["delmeplease",_bodynameig17g,false];
            _spawnAISg17g addEventHandler ["Fired", {_this call player_fired;}];
			_spawnAISg17g addEventHandler ["Killed",{_this call delete_guardia;}];
			_spawnAISg17g disableConversation true;
			_spawnAISg17g setdamage 0.01;
			
        };     
    };
   
    diag_log("DEBUG: Spawning " + str(_rndCountg17g) + " Taviana Guards from Group g17g at position " + str(_positiong17g));
	diag_log("Taviana Guards spawn Group g17g end");
};
*/
//raid start
raid_helis = {
	...
	//[65,[17686.6,7274.57,0.002]]
	//[114,[17677.5,7291.58,0.001]]
   ...
   _modelarrayg14g = ["SectorB_def","SectorB_def","SectorB_def","SectorB_def"];
    ...
    //_position = [8080,6618];
	//_positiong14g = [22870.3,19868.4];
    _rndCountg14g = 9; //ceil((random 3) + 1); // no soldiers
    //_rnd = random 1; // chance of highend loot on leader
    for "_x" from 1 to _rndCountg14g do {
        _bodynameig14g = format ["TavianaGuardg14g%1",_x];
	   
        if(_x == 1) then {
			_soldermodelg14g = _modelarrayg14g select 0; //(round(random ((count _modelarrayg14g) - 1)));
			...
			//_spawnTaviGroupLeaderg14g setDir (random(360));
			_spawnTaviGroupLeaderg14g setdamage 0.01;
			//_spawnTaviGroupLeaderg14g moveInCommander _object1AI1;
			_spawnTaviGroupLeaderg14g moveInTurret [_object1AI1, [0]];
		    
		   
            //waituntil {!isnil "bis_fnc_init"};
            //[_AISgroupg14g, [(_position select 0),(_position select 1),0], 450] call bis_fnc_taskPatrol;
			//[_AISgroupg14g, [(_position select 0),(_position select 1),0]] call BIS_fnc_taskDefend;
			//null = [_AISgroupg14g, [(_position select 0),(_position select 1),0]] execVM "\z\addons\dayz_server\system\BIN_taskDefend.sqf";
			...
			//_spawnAISg14g setDir (random(360));
        };     
    };
...
diag_log format ["1b=%1,2b=%2,3b=%3,1g=%4,2g=%5",bb_base_level1b,bb_base_level2b,bb_base_level3b,bb_base_level1g,bb_base_level2g]; 


diag_log "Armory start";

_io1 = [] call start_guardsg01g;
//diag_log format ["_io1=%1",_io1];
//_io2 = [] call start_guards2;
//_io3 = [] call start_guards3;
//_io4 = [] call start_guards4;

//_io5 = [] call start_guards5;
//_io6 = [] call start_guards6;
//_io7 = [] call start_guards7;
//_io8 = [] call start_guards8;
//_io9 = [] call start_guards9;
//_io10 = [] call start_guards10;
//_iog10g = [] call start_guardsg10g;
//_iog11g = [] call start_guardsg11g;
//_iog12g = [] call start_guardsg12g;
_iog13g = [] call start_guardsg13g;

_iog14g = [] call raid_helis;
//_iog14_2g = [] call start_guardsg17g;

_io1c = [] call start_civilian1c; //[76,[22344.3,19455.6,0]]
//_io2c = [] call start_civilian2c; //[76,[22344.3,19455.6,0]]
