...
if((speed player <= 1) && hasBagItem && _canDo) then {
    if (s_player_selfBloodbag < 0) then {
        s_player_selfBloodbag = player addaction[("<t color=""#c70000"">" + ("Self Bloodbag") +"</t>"),"actions\s_player_selfBloodbag.sqf","",5,false,true,"", ""];
    };
} else {
...
if (!isNull _cursorTarget and !_inVehicle and (player distance _cursorTarget < 4)) then { //Has some kind of target
	...
	_isTagbar = ((isPlayer cursorTarget) && !((getPlayerUID cursorTarget) in tag_friendlies) && !((getPlayerUID cursorTarget) in tag_req) && (alive cursorTarget));
	_isVehicle = _cursorTarget isKindOf "AllVehicles";
	_isMan = _cursorTarget isKindOf "Man";
	_isSafe = _cursorTarget isKindOf "TKOrdnanceBox_EP1";
	...
	if (_isSafe) then {
		if (s_safecargo < 0) then {
			s_safecargo = player addAction [("<t color=""#FFFFFF"">" + ("Cargo Check") + "</t>"), "actions\s_safecargo.sqf",cursorTarget, 1, false, true, "",""];
		};
	};
	...
	if (_isTagbar) then {
		if (s_tagfriendly < 0) then {
			s_tagfriendly = player addAction [("<t color=""#FF00FF"">" + ("Tag as Friend") + "</t>"), "actions\s_tagfriendly.sqf",cursorTarget, 1, false, true, "",""];
		};
	};
	//gather
...
//Fireplace Actions check
    ...
    if (_isMan and !_isAlive and !_isZombie and !_isAnimal) then {
		if (s_clothes < 0) then {
            s_clothes = player addAction [("<t color=""#08088A"">" + ("Take Clothes") + "</t>"), "actions\s_clothes.sqf",cursorTarget, 1, false, true, "",""];
        };
    } else {
