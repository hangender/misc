private ["_bloodAmount","_humanityBool","_infectionChance","_humanityNegBool","_humanityNegAmount","_humanityAmount","_infectedLifeLost","_infectedLifeBool","_lastBloodbag","_bloodbagLastUsedTime","_bloodbagTime","_bloodbagUseTime","_bloodbagUsageTime","_failChance"];
...
if (!dayz_canDisconnect) then { 
	...
	if (random(_failChance) < 1) then {
		player removeMagazine "ItemBloodbag";
		player removeAction s_player_selfBloodbag;
		player playActionNow "Medic"; 
		cutText [format["Self Bloodbag failed."], "PLAIN DOWN"]; 
	} else {
