...
if(isPlayer _aikiller) then {
	 private["_playerName","_playerWep","_dist"];
	 _playerName = name _aikiller;
	 _playerWep = primaryWeapon _aikiller;
	 _distance = round (_aikiller distance _ai);
     [_playerName,_playerWep,_distance, _aikiller] execVM "\z\addons\dayz_server\scripts\ai_kill.sqf";
};
...
if(isPlayer _aikiller) then {
     _humankills = _aikiller getVariable["npcKills",0];
     _aikiller setVariable["npcKills",_humankills+1,true];   
